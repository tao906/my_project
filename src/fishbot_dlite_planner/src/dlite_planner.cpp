...
nav_msgs::msg::Path downsamplePath(const nav_msgs::msg::Path& input_path, double min_dist = 0.8) {
...
nav_msgs::msg::Path smoothPathBSpline(const nav_msgs::msg::Path& input_path, double resolution = 0.05) {
    ...
    // ==========================================================
    // 核心优化：控制点张力松弛 (Control Point Relaxation)
    // 使用拉普拉斯平滑算子，像橡皮筋一样把连续的折角“拉平”，融合成一个大转角
    // ==========================================================
    std::vector<geometry_msgs::msg::PoseStamped> ctrl_pts = input_path.poses;
    int smooth_iters = 4; // 迭代4次，产生巨大的融合张力
    
    for (int iter = 0; iter < smooth_iters; ++iter) {
        std::vector<geometry_msgs::msg::PoseStamped> temp = ctrl_pts;
        // 掐头去尾，保证路径的绝对起点和终点坐标不发生偏移
        for (size_t i = 1; i < ctrl_pts.size() - 1; ++i) {
            // 1-2-1 高斯加权平均，极速抹平 S 型弯道
            temp[i].pose.position.x = 0.25 * ctrl_pts[i-1].pose.position.x + 
                                      0.50 * ctrl_pts[i].pose.position.x + 
                                      0.25 * ctrl_pts[i+1].pose.position.x;
            temp[i].pose.position.y = 0.25 * ctrl_pts[i-1].pose.position.y + 
                                      0.50 * ctrl_pts[i].pose.position.y + 
                                      0.25 * ctrl_pts[i+1].pose.position.y;
        }
        ctrl_pts = temp;
    }
    ...
    smoothed_path.poses.push_back(ctrl_pts.front());

    // 使用融合后的大轮廓控制点生成曲线
    for (size_t i = 0; i < ctrl_pts.size() - 3; ++i) {
        auto p0 = ctrl_pts[i];
        auto p1 = ctrl_pts[i+1];
        auto p2 = ctrl_pts[i+2];
        auto p3 = ctrl_pts[i+3];
    ...
    // 尾部闭合防抖
    if (ctrl_pts.size() >= 2) {
         auto p_penultimate = ctrl_pts[ctrl_pts.size()-2];
         ...
         smoothed_path.poses.push_back(ctrl_pts.back());
    }
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  // 自适应平方反比势场尺度参数
  nav2_util::declare_parameter_if_not_declared(node, name + ".potential_scale", rclcpp::ParameterValue(2.0));
  node->get_parameter(name + ".potential_scale", potential_scale_);

  nav2_util::declare_parameter_if_not_declared(node, name + ".obstacle_weight", rclcpp::ParameterValue(5.0));
  ...
  nav2_util::declare_parameter_if_not_declared(node, name + ".use_field_dstar_interpolation", rclcpp::ParameterValue(true));
  ...
  RCLCPP_INFO(node->get_logger(), 
    "DLitePlanner: Configured. Potential: %s (Scale: %.1f), BSplineSmooth: %s, TurnPenalty: %s, FieldInterp: %s", 
    use_potential_field_ ? "ON" : "OFF", potential_scale_,
...
double DLitePlanner::getExtraCost(const Node* n) {
    ...
    if (use_potential_field_) {
        if (is_static_map_ready_ && !static_distance_map_.empty()) {
            float dist_to_wall_cells = static_distance_map_[n->index];
            if (dist_to_wall_cells > 0) {
                // 转换为物理距离
                double dist_meters = dist_to_wall_cells * costmap_->getResolution();
                // 核心自适应公式：平方反比定律 (无论走廊多宽，最低点永远在中心线)
                extra += potential_scale_ / (dist_meters * dist_meters + 0.05); 
            }
        }
    } 
  ...
  const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  // 每次发送新目标，强制刷新静态势场地图
  if (use_potential_field_) {
      if (!is_static_map_ready_ || goal_changed) {
  ...
  if (use_bspline_smoothing_ && path.poses.size() > 5) {
      // 🌟 将采样距离提升至 0.8m，为合成大圆弧提供足够的跨度
      nav_msgs::msg::Path sparse_path = downsamplePath(path, 0.8); 
      final_path = smoothPathBSpline(sparse_path, 0.05);
  } else {
      for (size_t i = 0; i < final_path.poses.size() - 1; ++i) {
         double dx = final_path.poses[i+1].pose.position.x - final_path.poses[i].pose.position.x;
         double dy = final_path.poses[i+1].pose.position.y - final_path.poses[i].pose.position.y;
         if (std::hypot(dx, dy) > 1e-3) {
             double yaw = std::atan2(dy, dx);
             final_path.poses[i].pose.orientation.z = std::sin(yaw / 2.0);
             final_path.poses[i].pose.orientation.w = std::cos(yaw / 2.0);
         } else if (i > 0) {
             final_path.poses[i].pose.orientation = final_path.poses[i-1].pose.orientation;
         } else {
             final_path.poses[i].pose.orientation.w = 1.0;
         }
      }
      if(final_path.poses.size() > 1) {
          final_path.poses.back().pose.orientation = final_path.poses[final_path.poses.size()-2].pose.orientation;
      }
  }
